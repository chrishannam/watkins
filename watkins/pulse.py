...
from watkins.config import load_config
from watkins.connectors.egress.influxdb.processor import InfluxDBConnector
...
if __name__ == "__main__":
    print("starting!")
    config = load_config()
    influx_conn = InfluxDBConnector(config.influxdb)

    sensor_reader = SerialSensor(port=_detect_port())
    while True:
        reading = sensor_reader.read()
        if reading and reading < 190:
            influx_conn.write(reading)
